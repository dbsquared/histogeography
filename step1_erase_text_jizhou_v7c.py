#!/usr/bin/env python3
"""
v7c 白线追踪 — 轮廓吸附法（Contour Snapping）

核心思路：
1. 青色掩膜 → 给出冀州的大致形状和拓扑（保证面积正确）
2. 侧判定白线边界像素 → 给出精确的边界线位置（避免路网/文字干扰）
3. 对青色掩膜的每个轮廓点，找最近的白线边界像素 → 吸附到白线上
4. 高斯平滑坐标消除抖动

优势：
- 不依赖白线的连通性（即使断成几百片也无所谓）
- 形状由青色掩膜决定（不会出现手指、缺块等拓扑错误）
- 位置由白线决定（不会跟着路网跑）
"""
...
OUTDIR = os.path.join(HERE, 'jizhou_step1_v7c')
...
# 轻量清理青色掩膜（去掉文字/道路造成的小孔洞和突起）
kc9 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(9,9))
ko7 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(7,7))
cyan_clean = cv2.morphologyEx(cyan_mask, cv2.MORPH_CLOSE, kc9, iterations=1)
cyan_clean = cv2.morphologyEx(cyan_clean, cv2.MORPH_OPEN, ko7, iterations=1)
print(f'Cleaned cyan: {(cyan_clean>0).sum():,} px')

imwrite_safe(os.path.join(OUTDIR,'00_cyan_mask.png'), cyan_clean)

# ================================================================
# Phase 2: 白线侧判定（位置参考）
# ================================================================
lo_w = (r>210)&(g>210)&(b>210)
hi_w = (r>185)&(g>185)&(b>185)&(np.abs(r-g)<20)&(np.abs(g-b)<20)&(np.abs(r-b)<20)
white_any = lo_w|hi_w
...
# Phase 3: 青色掩膜外轮廓
# ================================================================
contours,_ = cv2.findContours(cyan_clean,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
...
# 微裁剪：去掉明显超出青色的孤立离群点（<2000px的独立连通分量）
