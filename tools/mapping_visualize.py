...
# ── 加载底图(降采样) ──
print('加载底图...')
base = Image.open(os.path.join(BASE_DIR, 'rendered/china_full_v3.png')).convert('RGB')
VW = 2600
scale = VW / BASE_W
VH = int(BASE_H * scale)
...
# ════════════════════════════════════════════════
print('\n=== 图2: GCP控制点 ===')
with open(os.path.join(BASE_DIR, 'tools/data/gcp_calibration.json'), encoding='utf-8') as f:
    cal = json.load(f)
